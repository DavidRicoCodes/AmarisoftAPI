...
try:
    ...
    print(f"No cell name detected")
    flag_cell_name=0

try:
    bandwidth = data["radio_config"]["bandwidth"]
except KeyError as e:
    print(f"No bandwidth especified")
    sys.exit(1)
...
if flag_db == 1:
    if cell_name not in cell_database:
        # Add new cell with bandwidth-specific SSB info
        ...
        # Check and update bandwidth-specific SSB info
...
if flag_cell_name == 0 and flag_cell_data ==0:
    print(f"No cell data or cell name especified")
    sys.exit(1)
...
# Write nr-erc.cfg to the specified output directory
...
# Generate users-scenario.cfg content
ue_list = []
for idx, command_entry in enumerate(commands, start=1):
    ...
    # Prepare sim_events based on the type of command
    if command_list[0] == "ping":
        # Configure for ping command
        sim_events = f"""[
      {{
        "start_time": 5,
        "event": "power_on"
      ...
      {{
        ...
        # Convert command arguments to a properly formatted args string for iperf
        iperf_args = [f'"{arg}"' for arg in command_list[1:]]  # Enclose each argument in double quotes
      ...
      {{
        ...
        end_time: {duration + 10},
        prog: "ext_app.sh",
        args: ["iperf3", {args_str},"-V","--debug"]
      }}
    ]"""
    
    # Append user entry with the configured sim_events
    ue_list.append(f"""  {{
    "ue_id": {ue_id},
    "imsi": "{imsi}",
...
# Join all user entries to form the complete ue_list for users-scenario.cfg
ue_list_content = ",\n".join(ue_list)  # Prepare the content separately to avoid issues

users_scenario_content = f"""ue_list: [
{ue_list_content}
]
"""

# Write users-scenario.cfg to the specified output directory
...
# Write ext_app.sh to the specified output directory
...
try:
    ...
    os.chmod(output_file_3, 0o755)  # Make the script executable
...
try:
    ...
    os.chmod(output_file_4, 0o755)  # Make the script executable
